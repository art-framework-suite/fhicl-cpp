...
BOOST_AUTO_TEST_CASE(GoodTuple1)
{
  ...
  BOOST_TEST(std::get<0>(pars[0]) == "A"s);
  BOOST_TEST(std::get<1>(pars[0]) == ref1);
  BOOST_TEST(std::get<0>(pars[1]) == "X"s);
  BOOST_TEST(std::get<1>(pars[1]) == ref2);
  auto const& defaultedPars = validatedConfig().nestedWithDef().pars();
  BOOST_TEST_REQUIRE(defaultedPars.size() == 1ull);
  BOOST_TEST(std::get<0>(defaultedPars[0]) == "T"s);
  BOOST_TEST(std::get<1>(defaultedPars[0]) == ref3);
}
...
BOOST_AUTO_TEST_CASE(GoodTuple2)
{
  ...
  BOOST_TEST(std::get<0>(defaultedPars[0]) == "M"s);
  BOOST_TEST(std::get<1>(defaultedPars[0]) == ref3);
  BOOST_TEST(std::get<0>(defaultedPars[1]) == "R"s);
  BOOST_TEST(std::get<1>(defaultedPars[1]) == ref4);
}
...
BOOST_AUTO_TEST_CASE(BadTuple1)
{
  ...
  BOOST_TEST_REQUIRE(NameStackRegistry::empty());
}
